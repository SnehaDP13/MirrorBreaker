...
(() => {
  ...
  // src/lib/mirrorbreaker/signal-utils.ts
  var TimeSeries = class {
    ...
    sampleRate() {
      ...
      const dur = (this.times[this.times.length - 1] - this.times[0]) / 1e3;
      if (dur <= 0) return 0;
  ...
  function detrend(x2) {
    if (!x2.length) return x2;
    const mean = x2.reduce((s2, v2) => s2 + v2, 0) / x2.length;
    return x2.map((v2) => v2 - mean);
  }
  function hann(n2) {
    const w2 = new Array(n2);
    for (let i2 = 0; i2 < n2; i2++) w2[i2] = 0.5 * (1 - Math.cos(2 * Math.PI * i2 / (n2 - 1)));
    return w2;
  }
  function goertzelPower(x2, targetFreq, sampleRate) {
    const N2 = x2.length;
    const k2 = Math.round(N2 * targetFreq / sampleRate);
    const omega = 2 * Math.PI * k2 / N2;
    const coeff = 2 * Math.cos(omega);
    let s0 = 0;
    let s1 = 0;
    let s2 = 0;
    for (let i2 = 0; i2 < N2; i2++) {
      s0 = x2[i2] + coeff * s1 - s2;
      s2 = s1;
      s1 = s0;
    }
  ...
  function estimateDominantFrequency(signal, sampleRate, fMin, fMax, steps = 120) {
    ...
    const windowed = detrended.map((v2, i2) => v2 * win[i2]);
    let bestFreq = 0;
    let bestPower = -Infinity;
    let sum = 0;
    for (let i2 = 0; i2 <= steps; i2++) {
      const f2 = fMin + (fMax - fMin) * i2 / steps;
      const p2 = goertzelPower(windowed, f2, sampleRate);
      sum += p2;
      if (p2 > bestPower) {
        bestPower = p2;
        bestFreq = f2;
  ...
  function std(x2) {
    if (x2.length < 2) return 0;
    const m2 = x2.reduce((s2, v3) => s2 + v3, 0) / x2.length;
    const v2 = x2.reduce((s2, v22) => s2 + (v22 - m2) * (v22 - m2), 0) / x2.length;
    return Math.sqrt(v2);
  }
  function clamp(x2, lo2, hi2) {
    return Math.max(lo2, Math.min(hi2, x2));
  }
  var EMA = class {
    constructor(alpha) {
      this.alpha = alpha;
    ...
    update(x2) {
      this.value = this.value == null ? x2 : this.alpha * x2 + (1 - this.alpha) * this.value;
      return this.value;
    }
    get() {
      return this.value ?? 0;
  ...
  function eyeAspectRatio(landmarks, idx) {
    ...
    const v2 = dist2D(p2, p6) + dist2D(p3, p5);
    const h2 = 2 * dist2D(p1, p4);
    return h2 > 0 ? v2 / h2 : 0;
  }
  function landmarksBBox(landmarks, indices, width, height) {
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
  ...
  function mouthOpenness(landmarks) {
    ...
    let acc = 0;
    let n2 = 0;
    for (const [a2, b2] of MOUTH_OPEN_PAIRS) {
      const pa2 = landmarks[a2];
      const pb = landmarks[b2];
      if (!pa2 || !pb) continue;
      acc += dist2D(pa2, pb);
      n2++;
    }
  ...
  // extension/iframe.ts
  var RPPG_WINDOW_MS = 1e4;
  var BLINK_WINDOW_MS = 3e4;
  var LIPSYNC_WINDOW_MS = 4e3;
  var landmarker = null;
  var greenSeries = new TimeSeries(RPPG_WINDOW_MS);
  var earSeries = new TimeSeries(BLINK_WINDOW_MS);
  var mouthSeries = new TimeSeries(LIPSYNC_WINDOW_MS);
  var audioSeries = new TimeSeries(LIPSYNC_WINDOW_MS);
  var blinkTimes = [];
  ...
  var bpmEl = document.getElementById("mb-bpm");
  var blinkEl = document.getElementById("mb-blink");
  var lipSyncEl = document.getElementById("mb-lipsync");
  var barEl = document.getElementById("mb-trust-bar");
  ...
  var inpBlinkLow = document.getElementById("cfg-blink-low");
  var inpBlinkHigh = document.getElementById("cfg-blink-high");
  var inpLipSync = document.getElementById("cfg-lipsync");
  var inpRppg = document.getElementById("cfg-rppg");
  function pushAlert(level, code, message) {
    const now = performance.now();
    const last = lastAlertTs[code] ?? 0;
    if (now - last < 4e3) return;
    lastAlertTs[code] = now;
    const alert = { time: (/* @__PURE__ */ new Date()).toISOString(), level, code, message };
    allAlerts.push(alert);
    if (alertsFeed) {
      const el = document.createElement("div");
      el.className = `alert-item ${level === "warn" ? "alert-warn" : level === "critical" ? "alert-crit" : ""}`;
      el.innerText = `[${(/* @__PURE__ */ new Date()).toLocaleTimeString()}] ${message}`;
      alertsFeed.prepend(el);
    }
  }
  function updateOverlay(bpm, blinkRate, lipSyncDrift, score, threatLevel) {
    if (statusEl) {
    ...
    let v2 = 0;
    if (threatLevel === "suspicious") v2 = 1;
    else if (threatLevel === "deepfake") v2 = 2;
    if (v2 > maxThreatScore) maxThreatScore = v2;
  }
  ...
  btnExport?.addEventListener("click", () => {
    const report = {
      ...
      durationSeconds: Math.floor((Date.now() - sessionStartTime) / 1e3),
    ...
    a2.href = url;
    a2.download = `mb-ext-report-${Date.now()}.json`;
    a2.click();
  ...
  async function initModel() {
    ...
    try {
      ...
      const fileset = await Zo.forVisionTasks(wasmUrl);
      landmarker = await Sc.createFromOptions(fileset, {
  ...
  function analyzeFrame(imageData, w2, h2, now) {
    ...
    if (analyser && audioDataArray) {
      ...
      let sumSquares = 0;
      for (let i2 = 0; i2 < audioDataArray.length; i2++) {
        const norm = audioDataArray[i2] / 128 - 1;
    ...
    if (faceDetected) {
      ...
      const foreheadBox = landmarksBBox(landmarks, FOREHEAD_LANDMARKS, w2, h2);
      ...
      if (earCurrent > recentEarMax) {
        ...
        recentEarMax = recentEarMax * 0.999 + earCurrent * 1e-3;
      }
    ...
    const blinkWinSec = Math.min(BLINK_WINDOW_MS, now - (blinkTimes[0] ?? now)) / 1e3;
    ...
    const audMean = audVals.reduce((s2, v2) => s2 + v2, 0) / Math.max(1, audVals.length);
    if (audMean > 0.015 && audVals.length > 30 && mouthVals.length > 30) {
      const N2 = Math.min(audVals.length, mouthVals.length, 120);
      const a2 = audVals.slice(-N2);
      const m2 = mouthVals.slice(-N2);
      const mouthVar = std(m2);
      const maxMouth = Math.max(...m2);
      if (mouthVar < 5e-3 && maxMouth < 0.06) {
        lipSyncDrift = 0.95;
      } else {
        const aMax = Math.max(...a2, 1e-3);
        const aNorm = a2.map((v2) => v2 / aMax);
        const mMin = Math.min(...m2);
        const mMax = Math.max(...m2, mMin + 1e-3);
        const mNorm = m2.map((v2) => (v2 - mMin) / (mMax - mMin));
        const smooth = (arr) => {
          const out = [];
          for (let i2 = 0; i2 < arr.length; i2++) {
            let sum = 0, count = 0;
            for (let j2 = Math.max(0, i2 - 2); j2 <= Math.min(arr.length - 1, i2 + 2); j2++) {
              sum += arr[j2];
              count++;
            }
            out.push(sum / count);
        ...
        let mse = 0;
        for (let i2 = 0; i2 < N2; i2++) {
          const diff = aSmooth[i2] - mSmooth[i2];
          mse += diff * diff;
        }
        const rmse = Math.sqrt(mse / N2);
        lipSyncDrift = clamp((rmse - 0.2) * 2, 0, 1);
      }
    }
    let score = 100;
    if (!faceDetected) {
